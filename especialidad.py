...
class EspecialidadApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Gestión de Especialidades")
    ...
    def create_data_entry_widgets(self, frame):
        labels = ["ID Especialidad", "Nombre Especialidad", "Diseño Curricular", "Cantidad Materias"]
        self.entry_vars = {}
    ...
    def create_treeview_widget(self, frame):
        self.tree = ttk.Treeview(frame, columns=("ID Especialidad", "Nombre Especialidad", "Diseño Curricular", "Cantidad Materias"))
        ...
        self.tree.heading("#4", text="Cantidad Materias")
        self.tree.grid()
        self.tree.column("#0", width=2)
        self.tree.column("#1", width=100)
        self.tree.column("#2", width=200)  # nombre_especialidad
        self.tree.column("#3", width=200)  # disenio_curricular
        self.tree.column("#4", width=200)  # cantidad_materia
    ...
    def agregar(self):
        ...
        try:
            ...
            cursor.execute("INSERT INTO especialidad (nombre_especialidad, disenio_curricular, cantidad_materias) "
                           "VALUES (?, ?, ?)",
                           (datos["Nombre Especialidad"], datos["Diseño Curricular"], datos["Cantidad Materias"]))
            self.conn.commit()
    ...
    def load_data_to_entries(self, event):
        ...
        if selected_item:
            ...
            keys_order = ["ID Especialidad", "Nombre Especialidad", "Diseño Curricular", "Cantidad Materias"]
    ...
    def borrar_registro(self):
        id_a_borrar = self.entry_vars["ID Especialidad"].get()

        if not id_a_borrar:
            messagebox.showerror("Error", "Ingrese un id_especialidad para borrar un registro.")
            return

        try:
            cursor = self.conn.cursor()
            cursor.execute("DELETE FROM especialidad WHERE id_especialidad=?", (id_a_borrar,))
            self.conn.commit()
    ...
    def modificar_registro(self):
        datos = {label: var.get() for label, var in self.entry_vars.items()}
        id_a_modificar = datos["ID Especialidad"]

        if not id_a_modificar:
            messagebox.showerror("Error", "Ingrese un id_especialidad para modificar un registro.")
            return

        try:
            cursor = self.conn.cursor()
            cursor.execute("UPDATE especialidad SET nombre_especialidad=?, disenio_curricular=?, cantidad_materias=? WHERE id_especialidad=?",
                           (datos["Nombre Especialidad"], datos["Diseño Curricular"], datos["Cantidad Materias"], id_a_modificar))
            self.conn.commit()
    ...
    def consultar_datos(self):
        # Obtener el ID de la especialidad a consultar desde el campo de entrada
        id_especialidad_consulta = self.entry_vars["ID Especialidad"].get()
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute("SELECT id_especialidad, nombre_especialidad, disenio_curricular, cantidad_materias FROM especialidad WHERE id_especialidad=?", (id_especialidad_consulta,))
