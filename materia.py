...
class MateriaApp:
    ...
    def create_widgets(self):
        ...
        frame_derecha = tk.Frame(self.root, width=400, height=400, bg="DodgerBlue4")
    ...
    def create_data_entry_widgets(self, frame):
        labels = ["ID Materia", "Nombre Materia", "ID Especialidad"]
        self.entry_vars = {}
    ...
    def create_treeview_widget(self, frame):
        self.tree = ttk.Treeview(frame, columns=("ID Materia", "Nombre Materia", "ID Especialidad"))
    ...
    def agregar(self):
        ...
        try:
            ...
            cursor.execute("INSERT INTO materia (nombre_materia, id_especialidad) "
                           "VALUES (?, ?)",
                           (datos["Nombre Materia"], datos["ID Especialidad"]))
            self.conn.commit()
    ...
    def load_data_to_entries(self, event):
        ...
        if selected_item:
            ...
            keys_order = ["ID Materia", "Nombre Materia", "ID Especialidad"]
    ...
    def borrar_registro(self):
        id_materia_a_borrar = self.entry_vars["ID Materia"].get()
    ...
    def modificar_registro(self):
        datos = {label: var.get() for label, var in self.entry_vars.items()}
        id_materia_a_modificar = datos["ID Materia"]
        ...
        try:
            ...
            cursor.execute("UPDATE materia SET nombre_materia=?, id_especialidad=? WHERE id_materia=?",
                           (datos["Nombre Materia"], datos["ID Especialidad"], id_materia_a_modificar))
            self.conn.commit()
    ...
    def consultar_datos(self):
        # Obtener el ID de Materia a consultar desde el campo de entrada
        id_materia_consulta = self.entry_vars["ID Materia"].get()
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute("SELECT id_materia, nombre_materia, id_especialidad FROM materia WHERE id_materia=?", (id_materia_consulta,))
