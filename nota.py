...
class NotasApp:
    ...
    def create_data_entry_widgets(self, frame):
        labels = ["ID Nota", "Valor", "Año de Cursada", "Instancia", "Fecha de Nota", "ID Alumno", "ID Materia"]
        self.entry_vars = {}
    ...
    def create_treeview_widget(self, frame):
        self.tree = ttk.Treeview(frame, columns=("ID Nota", "Valor", "Año de Cursada", "Instancia", "Fecha de Nota", "ID Alumno", "ID Materia"))
        self.tree.heading("#1", text="ID Nota")
        self.tree.heading("#2", text="Valor")
        self.tree.heading("#3", text="Año de Cursada")
        self.tree.heading("#4", text="Instancia")
        self.tree.heading("#5", text="Fecha de Nota")
    ...
    def agregar(self):
        ...
        try:
            ...
            cursor.execute("INSERT INTO nota (valor, annio_cursada, instancia, fecha_nota, id_alumno, id_materia) "
                           "VALUES (?, ?, ?, ?, ?, ?)",
                           (datos["Valor"], datos["Año de Cursada"], datos["Instancia"],
                            datos["Fecha de Nota"], datos["ID Alumno"], datos["ID Materia"]))
            self.conn.commit()
    ...
    def load_data_to_entries(self, event):
        ...
        if selected_item:
            ...
            keys_order = ["ID Nota", "Valor", "Año de Cursada", "Instancia", "Fecha de Nota", "ID Alumno", "ID Materia"]
    ...
    def borrar_registro(self):
        id_nota_a_borrar = self.entry_vars["ID Nota"].get()
    ...
    def modificar_registro(self):
        datos = {label: var.get() for label, var in self.entry_vars.items()}
        id_nota_a_modificar = datos["ID Nota"]
        ...
        try:
            ...
            cursor.execute("UPDATE nota SET valor=?, annio_cursada=?, instancia=?, fecha_nota=?, id_alumno=?, id_materia=? WHERE id_nota=?",
                           (datos["Valor"], datos["Año de Cursada"], datos["Instancia"],
                            datos["Fecha de Nota"], datos["ID Alumno"], datos["ID Materia"], id_nota_a_modificar))
            self.conn.commit()
    ...
    def consultar_datos(self):
        id_nota_consulta = self.entry_vars["ID Nota"].get()
        if not id_nota_consulta:
            messagebox.showerror("Error", "Ingrese un ID de Nota para buscar un registro.")
            return
        try:
            cursor = self.conn.cursor()
            cursor.execute("SELECT id_nota, valor, annio_cursada, instancia, fecha_nota, id_alumno, id_materia FROM nota WHERE id_nota=?", (id_nota_consulta,))
